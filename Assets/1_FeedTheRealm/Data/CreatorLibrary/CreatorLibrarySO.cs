...
public class CreatorLibrary : InitializableSO {
    ...
    [SerializeField] public string libraryFilePath = "Assets/models.json";
    [SerializeField] public string modelsDirectory = "Models";
    ...
    protected override void OnInitialize() {
        ...
        if (!System.IO.File.Exists(fullPath)) {
            logger.Log($"Library file not found at: {fullPath}. Generating new library. . .", this, Logging.LogType.Warning);
    ...
    private void LoadLibrary() {
        ...
        List<ObjectData> objects = JsonUtility.FromJson<ObjectDataList>(json).objects;
        ...
        logger.Log($"Loaded {objectData.Count} objects into the library from: {libraryFilePath}", this, Logging.LogType.Info);
    }

    private void GenerateLibrary(string outputPath) {
        List<ObjectData> objectData = new();
        string modelsPath = System.IO.Path.Combine(Application.streamingAssetsPath, modelsDirectory);
        if (!System.IO.Directory.Exists(modelsPath)) {
            logger.Log($"Models directory not found at: {modelsPath}", this, Logging.LogType.Error);
            return;
        }
        string[] objectFiles = System.IO.Directory.GetFiles(modelsPath, "*.*")
        ...
        // Write to JSON file
        ObjectDataList dataList = new() { objects = objectData };
        string json = JsonUtility.ToJson(dataList, true);
        System.IO.File.WriteAllText(outputPath, json);
        logger.Log($"Generated library with {objectData.Count} assets at: {outputPath}", this, Logging.LogType.Info);
    }
