...
public static class SpriteStorage
{
    ...
    public static string SaveFileReturnId(string sourceAbsolutePath)
    {
        ...
        {
            Debug.LogWarning(
                $"SpriteStorage: Failed to save file to persistent data: {ex.Message}"
            );
            return null;
        }
    }
    ...
    public static string GetFilePathFromIdOrPath(string idOrPath)
    {
        ...
        try
        {
            ...
            if (Path.IsPathRooted(idOrPath) && File.Exists(idOrPath))
        ...
        {
            Debug.LogWarning(
                $"SpriteStorage: Failed to resolve id/path '{idOrPath}': {ex.Message}"
            );
            return null;
        }
    }
    ...
    public static byte[] LoadSpriteBytesFromPath(string absolutePath)
    {
        ...
        {
            Debug.LogWarning(
                $"SpriteStorage: Failed to load sprite bytes from path '{absolutePath}': {ex.Message}"
            );
            return null;
        }
    }
