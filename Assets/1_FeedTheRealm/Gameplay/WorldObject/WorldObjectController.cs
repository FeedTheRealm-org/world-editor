...
public class WorldObjectController : MonoBehaviour {
    ...
    [Tooltip("When using local assets, this is the asset name without extension from the StreamingAssets folder. For remote assets, this is the full URL.")]
    [SerializeField] private string objectUrl;
    [SerializeField] private bool isLocalAsset = true;
    ...
    /// </summary>
    public static WorldObjectController SetupObject(
        string id,
        string assetUrl,
        Vector3 size,
        Vector3 position,
        ...
        var controller = new WorldObjectController {
            id = id,
            objectUrl = assetUrl,
            size = size,
            position = position,
    ...
    async void Awake() {
        ...
        if (instantiateObject) worldObject = gameObject;
        LoadObject();
        ApplyRootTransform();
