...
public class ShopMenuController : MenuController
{
    ...
    //     AddItemsByCategory(CreatableObjectCategories.WeaponItem);
    //     AddItemsByCategory(CreatableObjectCategories.ConsumableItem);
    //     itemSelector.SetValueWithoutNotify(ItemPlaceholder);
    // }

    // private void AddItemsByCategory(CreatableObjectCategories category)
    // {
    //     List<CreatorObject> items = creatorObjectLibrary.GetAll(category);
